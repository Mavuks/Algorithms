...
public class IntSorting {
   ...
   public static void binaryInsertionSort(int[] a) {
      ...
      for (int i = 1; i < a.length; i++) {
         ...
         //Binaarneotsing while loobiga.
         while (first < last) {
            int middle = first + (last - first) / 2; // Leiab array keskkoha.
            if (b >= a[middle]) {
               first = middle + 1;
            } else {
               last = middle;
         ...
         // Liigutab listi ühe kohta võrra paremale.
         System.arraycopy(a, first, a, first + 1, i - first);

         a[first] = b;
      }
   }
         //Kui Listi Suurus on väiksem kui 2 siis pole seal midagi muuta vaja.
         //https://www.geeksforgeeks.org/binary-insertion-sort/
//       if (a.length < 2)
//           return;
//       for (int i = 0; i < a.length; i++)  //Käime terve list läbi
//       {
//           int b = a[i];
//
//           // Otsib koha kuhu panna numbri, kasutades binaarsetotsingut.
//           int j = Math.abs(Arrays.binarySearch(a, 0, i, b) + 1);
//
//           // Liigutab listi ühe kohta võrra paremale.
//           System.arraycopy(a, j, a, j+1, i-j);
//
//           // Paneb elemendi õigesse kohta
//           a[j] = b;
//       }
